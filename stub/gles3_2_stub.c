...
static StubMapEntry *find_stub_map(GLuint buffer, GLenum target)
{
...
static bool mapped_index_bounds(const uint8_t *idx, GLsizei count, GLenum type,
                                GLint basevertex, GLintptr *min_vertex,
                                GLintptr *max_vertex)
{
  if (!idx || count <= 0)
    return false;
